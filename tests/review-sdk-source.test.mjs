...
test('admin menu DOM renders Zenith UI dots with text tooltips and rejects dangerous icon or url inputs', () => {
  ...
  assert.match(source, /\.za-menu-item \{[^}]*width: 12px; height: 12px/)
  assert.match(source, /\.za-menu-dot \{[^}]*width: 7px; height: 7px;[^}]*background: rgba\(209, 255, 244, 0\.82\); box-shadow: none/)
...
test('admin overlay exposes SDK-built Admin home navigation beside Open admin panel', () => {
  assert.match(source, /adminHomeLabel\?: string/)
  assert.match(source, /adminHomeUrl\?: string \| \(\(\) => string\)/)
  assert.match(source, /onAdminHomeSelect\?: \(url: string\) => void \| Promise<void>/)
  assert.match(source, /function getDefaultZenithAdminHomeUrl\(\): string \{/)
  assert.match(source, /return new URL\('admin', document\.baseURI \|\| window\.location\.href\)\.href/)
  assert.match(source, /<div class="za-actions">[\s\S]*<button class="za-action" data-action="open" type="button"><\/button>[\s\S]*<button class="za-action" data-action="admin-home" type="button"><\/button>/)
  assert.match(source, /adminHomeAction\.textContent = options\.adminHomeLabel \?\? 'Admin home'/)
  assert.match(source, /adminHomeAction\.hidden = !session/)
  assert.match(source, /adminHomeAction\.addEventListener\('click', \(\) => \{/)
  assert.match(source, /window\.location\.assign\(url\)/)
  assert.doesNotMatch(source, /data-action="admin-home" type="button">\$\{escapeReviewAuthHtml\(options\.adminHomeLabel \?\? 'Admin'\)\}/)
  assert.doesNotMatch(source, /let adminHomeButton: HTMLButtonElement \| null = null/)
})
