...
For full-page review/reporting flows, use `createReviewHud(...)` instead of embedding review controls into a host-app page. The HUD mounts itself into `document.body` with isolated Shadow DOM, restores the short-lived auth session from `sessionStorage`, starts/stops the global recorder, and submits through Hub without navigating the reviewed site. If `hubUrl` is omitted, the HUD submits to the production Hub at `https://hub.zenith-research.ca`; consumers may still pass an explicit override for local development. Cmd+Shift+Z should toggle SDK chrome visibility only; consumers should wire it to show/hide `renderZenithAdminOverlay(...)` and close the HUD, not to start recording.

Spoken reviews fail closed when microphone capture is unavailable. `createReviewHud(...)` enters active review mode only after the reviewer clicks **Start review** and the browser has granted and started microphone recording. `submitReview(...)` requires real recorded audio for spoken-review mode and does not synthesize blank fallback audio. Intentional silent or visual-only reviews must use an explicit future input mode rather than relying on missing audio.
