...
interface WindowWithZenithAdminShortcut {
...
function getZenithAdminShortcutState(): ZenithAdminShortcutState {
  const target = globalThis as unknown as WindowWithZenithAdminShortcut
...
function attachZenithAdminShortcutFallbackState(open: () => void, eventName: string) {
...
function detachZenithAdminShortcutFallbackState() {
  const windowObject = globalThis as unknown as WindowWithZenithAdminShortcut
...
function createZenithAdminShortcutKeyHandler(eventName: string): (event: KeyboardEvent) => void {
  return event => {
    if (event.key.toLowerCase() !== 'z' || !event.shiftKey || !(event.metaKey || event.ctrlKey)) return
    if (event.repeat) return
    if (isTextInputLikeElement(event.target)) return
...
export function initZenithAdminShortcut(options: ZenithAdminShortcutOptions): ZenithAdminShortcutHandle {
...
class BrowserReviewRecorder implements ReviewRecorder {
  ...
  private logScrollEvent() {
    const elapsed = performance.now() - this.startTime
    if (elapsed - this.lastScrollLogAt < 250) return
  ...
  private async startMediaCapture() {
    if (!this.options.captureAudio) return
...
interface ValidatedZenithAdminMenuItem extends ZenithAdminMenuItem {
  providerId: string
  id: string
  slot: number
  label: string
...
interface ZenithAdminMenuValidationContext {
  allowedProviders: Set<string>
  existingBySlot: Map<number, ValidatedZenithAdminMenuItem>
  countByProvider: Map<string, number>
  total: number
}

function normalizeZenithAdminMenuText(value: string, maxLength: number, field: string): string {
  const normalized = value.normalize('NFKC').trim()
  if (!normalized) throw new Error(`Zenith admin menu ${field} is required`)
  if (normalized.length > maxLength) throw new Error(`Zenith admin menu ${field} is too long`)
  return normalized
}

function assertZenithAdminMenuIdentifier(value: string, field: string): string {
  const normalized = normalizeZenithAdminMenuText(value, 64, field)
  if (!ZENITH_ADMIN_MENU_ID_PATTERN.test(normalized)) throw new Error(`Invalid Zenith admin menu ${field}`)
  return normalized
}

function normalizeZenithAdminMenuPermissions(permissions: string[] | undefined): string[] {
  if (!permissions || permissions.length === 0) return []
  return permissions.map(permission => {
    const normalized = normalizeZenithAdminMenuText(permission, 96, 'permission')
    if (!ZENITH_ADMIN_MENU_PERMISSION_PATTERN.test(normalized)) throw new Error('Invalid Zenith admin menu permission')
    return normalized
  })
}

function validateZenithAdminMenuItem(
  item: ZenithAdminMenuItem,
  context: ZenithAdminMenuValidationContext,
): ValidatedZenithAdminMenuItem {
  ...
  if (item.icon?.value) normalizeZenithAdminMenuText(item.icon.value, 8, 'icon')
...
function validateZenithAdminMenuItems(items: readonly ZenithAdminMenuItem[] | undefined, allowedProviders: Set<string>): ValidatedZenithAdminMenuItem[] {
  const existingBySlot = new Map<number, ValidatedZenithAdminMenuItem>()
  const countByProvider = new Map<string, number>()
  const validated: ValidatedZenithAdminMenuItem[] = []
  ...
  for (const item of items ?? []) {
    ...
    if (replaced) {
      ...
      if (index >= 0) validated.splice(index, 1, next)
    } else {
      validated.push(next)
...
function createZenithAdminMenuAuthSnapshot(session: ReviewAuthSession, permissions: readonly string[]): ZenithAdminMenuAuthSnapshot {
  return {
...
function createSafeZenithMenuEvent(event: MouseEvent | KeyboardEvent): ZenithSafeMenuEvent {
  return {
...
function hasZenithAdminMenuPermission(auth: ZenithAdminMenuAuthSnapshot, permission: string): boolean {
  return auth.permissions.includes(permission) || auth.permissions.includes('admin:*') || auth.permissions.includes(`${permission.split(':')[0]}:*`)
}

function canRenderZenithAdminMenuItem(item: ValidatedZenithAdminMenuItem, auth: ZenithAdminMenuAuthSnapshot): boolean {
...
function createZenithAdminOverlayStyles(): string {
  return `
    ...
    *, *::before, *::after { box-sizing: border-box; }
    ...
    .za-menu-dot { width: 10px; height: 10px; border-radius: 999px; border: 1px solid rgba(209, 255, 244, 0.82); background: rgba(209, 255, 244, 0.82); box-shadow: none; transition: transform 150ms ease, border-color 150ms ease, background 150ms ease; }
...
export function renderZenithAdminOverlay(options: ZenithAdminOverlayOptions): ZenithAdminOverlayHandle {
  ...
  const activeMenuControllers = new Map<string, AbortController>()
  const host = document.createElement('div')
  const shadow = host.attachShadow({ mode: 'closed' })
  const style = document.createElement('style')
  ...
  const mainButton = root.querySelector('.za-button') as HTMLButtonElement
  ...
  function getCurrentAuthSnapshot(currentSession: ReviewAuthSession): ZenithAdminMenuAuthSnapshot {
  ...
  function renderMenuItems(currentSession: ReviewAuthSession | null) {
    ...
    if (options.onAdminHomeSelect) {
      ...
      homeActionButton.addEventListener('click', event => {
        ...
        const runAllowedOperation = async <T>(operation: ZenithAllowedAdminOperation<T>): Promise<T> => {
          if (controller.signal.aborted) throw new Error('Zenith admin home action was aborted')
          ...
          if (operation.signal?.aborted) throw new Error('Zenith admin home operation was aborted')
        ...
        const context: ZenithAdminHomeActionContext = {
          auth: latestAuth,
          actions: {
            closeMenu: () => {
              open = false
        ...
        Promise.resolve(options.onAdminHomeSelect?.(context)).catch(error => {
          ...
          if (generationAtStart === sessionGeneration && !controller.signal.aborted) homeActionButton.disabled = false
        })
      })
      homeActionRoot.replaceChildren(homeActionButton)
    }
    const stateFor = (item: ValidatedZenithAdminMenuItem): ZenithAdminMenuState => ({
      auth,
      item: { providerId: item.providerId, id: item.id, slot: item.slot },
    })

    const visibleItems = validatedMenuItems.filter(item => {
      if (!canRenderZenithAdminMenuItem(item, auth)) return false
    ...
    const renderItem = (item: ValidatedZenithAdminMenuItem) => {
      ...
      if (item.icon?.kind === 'text') dot.dataset.icon = item.icon.value
      ...
      itemButton.addEventListener('click', event => {
        ...
        const runAllowedOperation = async <T>(operation: ZenithAllowedAdminOperation<T>): Promise<T> => {
          if (controller.signal.aborted) throw new Error('Zenith admin menu action was aborted')
          const operationSession = options.manager.getSession()
          if (!operationSession || !isReviewAuthSessionFresh(operationSession) || generationAtStart !== sessionGeneration) {
            throw new Error('Zenith admin session changed before operation completed')
          }
          ...
          if (operation.signal?.aborted) throw new Error('Zenith admin menu operation was aborted')
          const result = await operation.run?.(controller.signal)
          if (controller.signal.aborted || generationAtStart !== sessionGeneration) {
            throw new Error('Zenith admin session changed before operation completed')
          }
        ...
        const context: ZenithAdminMenuActionContext = {
          auth: latestAuth,
          actions: {
            closeMenu: () => {
              open = false
        ...
        Promise.resolve(item.onSelect(context)).catch(error => {
          ...
          activeMenuControllers.delete(itemKey)
          if (generationAtStart === sessionGeneration && !controller.signal.aborted) itemButton.disabled = false
        })
      })
      return itemButton
    }
...
type ReviewHudStatus = 'idle' | 'starting' | 'recording' | 'submitting' | 'submitted' | 'error'
...
export function createReviewHud(options: ReviewHudOptions): ReviewHudHandle {
  ...
  function render() {
    ...
    if (closeButton) closeButton.disabled = status === 'starting' || status === 'submitting'
